...
class jgiveModelCampaign extends JModelLegacy
{
	...
	function save()
	{
		...
		$result=$dispatcher->trigger('OnAfterJGiveCampaignSave',array($camp_id));//Call the plugin and get the result
		if(!$result){
			//return false;
		}
	...
	function edit()
	{
		...
		$dispatcher=JDispatcher::getInstance();
		JPluginHelper::importPlugin('system');
		$result=$dispatcher->trigger('OnAfterJGiveCampaignEdit',array($camp_id));//Call the plugin and get the result
		if(!$result){
			//return false;
		}
	...
	function getCampaign()
	{
		...
		//get campaign donors
		$cdata['donors']=$campaignHelper->getCampaignDonors($cid);
		//print_r($cdata);
